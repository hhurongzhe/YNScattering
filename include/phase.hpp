...
std::vector<double> get_phase_uncoupled(const YN::YN_configs &configs, double p_lab_now)
{
    ...
    double temp1s0 = atan(factor * scattering::R_uncoupled(0, 0, 0, configs, p_lab_now));
    double temp3p0 = atan(factor * scattering::R_uncoupled(1, 1, 0, configs, p_lab_now));
    ...
    else
    {
        ...
        {
            tempp0 = atan(factor * scattering::R_uncoupled(j_temp, 0, j_temp, configs, p_lab_now));
            tempp1 = atan(factor * scattering::R_uncoupled(j_temp, 1, j_temp, configs, p_lab_now));
...
std::vector<double> get_phase_bar_coupled(const YN::YN_configs &configs, double p_lab_now)
{
    int de = configs.mesh_points_LS;
    ...
    else
    {
        for (int j_temp = 1; j_temp < jmax + 1; j_temp = j_temp + 1)
        {
            coupled = scattering::R_coupled_triplet(j_temp, configs, p_lab_now);
            rpp = coupled[0];
            rpm = coupled[2];
            rmm = coupled[3];
...
std::vector<std::string> partial_wave_name_uncoupled(const YN::YN_configs &configs)
{
...
std::vector<std::string> partial_wave_name_coupled(const YN::YN_configs &configs)
{
    std::vector<std::string> temp;
...
std::vector<double> get_phase_uncoupled_matched(const YN::YN_configs &configs, double p_lab_now)
{
    int de = configs.mesh_points_LS;
    std::vector<double> phase;
    ...
    double tempp0, tempp1;
    double temp1s0 = atan(factor * scattering::R_uncoupled(0, 0, 0, configs, p_lab_now));
    double temp1s0_matched = match_coulomb::match_phase_uncoupled(temp1s0, 0, configs, p_lab_now);
    double temp3p0 = atan(factor * scattering::R_uncoupled(1, 1, 0, configs, p_lab_now));
    double temp3p0_matched = match_coulomb::match_phase_uncoupled(temp3p0, 1, configs, p_lab_now);
    phase.push_back(temp1s0_matched);
    phase.push_back(temp3p0_matched);
    ...
    else
    {
        for (int j_temp = 1; j_temp < jmax + 1; j_temp = j_temp + 1)
        {
            // first calculate unmatched phase shifts, and then do the matching.
            tempp0 = atan(factor * scattering::R_uncoupled(j_temp, 0, j_temp, configs, p_lab_now));
            double tempp0_matched = match_coulomb::match_phase_uncoupled(tempp0, j_temp, configs, p_lab_now);
            tempp1 = atan(factor * scattering::R_uncoupled(j_temp, 1, j_temp, configs, p_lab_now));
            double tempp1_matched = match_coulomb::match_phase_uncoupled(tempp1, j_temp, configs, p_lab_now);
            phase.push_back(tempp0_matched);
            phase.push_back(tempp1_matched);
        }
    }
    return phase;
}

std::vector<double> get_phase_bar_coupled_matched(const YN::YN_configs &configs, double p_lab_now)
{
    int de = configs.mesh_points_LS;
    std::vector<double> phase_bar;
    double mu = configs.mass_proton * configs.mass_sigmap / (configs.mass_proton + configs.mass_sigmap);
    ...
    else
    {
        for (int j_temp = 1; j_temp < jmax + 1; j_temp = j_temp + 1)
        {
            ...
            rpm = coupled[2];
            rmm = coupled[3];
            std::vector<double> phase_matched =
                match_coulomb::match_phase_coupled(rpp, rpm, rmm, j_temp, configs, p_lab_now);
            delm = phase_matched[0];
            delp = phase_matched[1];
            epsb = phase_matched[2];
            phase_bar.push_back(delm);
            phase_bar.push_back(delp);
            phase_bar.push_back(epsb);
        }
