...
//
// If the number of iterations reaches 100000 and |z| > 0.5, the convergence is too slow. One is probably very close to
// the imaginary axis. If l=0 and |z| <= 0.5, the direct integration is still done as it is stable. If 1+l+/-i.eta is
// negative integer, it has to be done otherwise it is too long even if |z| is not exceedingly Small. One first
// considers Re[z] >= 0. One takes the starting point z0 = 2 + i.(Im[z] + 2.sign[Im[z]]) (0.6 +
// i.sign[Re[z]].0.6.sign[Im[z]] if |z| <= 0.5). Then, one calculates H[omega],H[omega]' at the starting point, and one
// integrates numerically H[omega] until z. h(omega)(z) is then H[omega](z)'/H[omega](z). If Re[z] < 0, one calculates
// H[-omega],H[-omega]' with l, eta -> -eta, and z -> -z. One uses the Coulomb wave functions class cwf_minus_eta_ptr
// defined with l and -eta. The ratio h(omega,l,eta,z) is then equal to -h(-omega,l,-eta,-z). To avoid infinite loops,
// continued_fraction_h must not be used in this integration. So, the starting point is chosen so |H[+/-]| is very
// likely to increase in modulus. Then, one always integrates forward. Forward integration is enforced putting
...
// debut_cwf,F_debut_cwf,dF_debut_cwf : values stored in cwf put back in cwf at the end of direct integration as they
// change values.
...
complex<double> Coulomb_wave_functions::continued_fraction_h(const complex<double> &z, const int omega)
{
    ...
    const complex<double> a = I_omega_eta + l + 1.0, c = I_omega_eta - l, z_minus_eta = z - eta,
    ...
    {
        const int nm1 = n - 1;
        const complex<double> an = (a + nm1) * (c + nm1), bn = two_z_minus_eta + n * two_I_omega,
                              bn_plus_an_Dn = bn + an * Dn, bn_plus_an_over_Cn = bn + an / Cn;
        ...
        if ((n++ > 100000) && ((l == 0.0) || (abs_z > 0.5) || neg_int_omega_one || neg_int_omega_minus_one))
        {
            if ((real(z) < 0.0) && (cwf_minus_eta_ptr == 0))
                cwf_minus_eta_ptr = new class Coulomb_wave_functions(is_it_normalized, l, -eta);
            class Coulomb_wave_functions &cwf = (real(z) < 0.0) ? (*cwf_minus_eta_ptr) : (*this);

            const complex<double> z00(2.0, SIGN(real(z)) * (imag(z) + 2.0 * SIGN(imag(z)))),
                z01(0.6, 0.6 * SIGN(real(z)) * SIGN(imag(z)));
            const complex<double> z_start = (abs_z > 0.5) ? (z00) : (z01), debut_cwf = cwf.debut,
                                  F_debut_cwf = cwf.F_debut, dF_debut_cwf = cwf.dF_debut;
            complex<double> F_start, dF_start, H, dH;
            cwf.F_dF(z_start, F_start, dF_start);
            cwf.is_H_dir_int_naive = true, cwf.H_dH_direct_integration(SIGN(real(z)) * omega, SIGN(real(z)) * z, H, dH),
...
// H[omega]'(z) = exp[i.omega.[z - eta.log[2z] - l.Pi/2 + sigma(l,eta)]].[S[omega]'(z) + i.omega.(1 -
...
// H[-omega][ASd] is given by the asymptotic series, which gives the good result H[-omega] in this case as one is not in
// its bad quadrant.
//
// The function is scaled, so one returns : H[omega](z).exp[-i.omega.[z - eta.log[2z]]] and H[omega]'(z).exp[-i.omega.[z
// - eta.log[2z]]] .
//
...
//                       If log_cut_constant_AS is not finite (i.e. cut_constant_AS = 0 if it is defined), there is no
...
void Coulomb_wave_functions::asymptotic_expansion_H_dH_scaled(const int omega, const complex<double> &one_over_z,
                                                              complex<double> &H_scaled, complex<double> &dH_scaled,
                                                              bool &is_it_successful)
{
    ...
    const complex<double> I_omega(0, omega), two_I_omega(0, 2 * omega),
        I_omega_one_minus_eta_over_z = I_omega * (1.0 - eta * one_over_z);

    const complex<double> phase_shift = I_omega * (sigma_l - l * 1.5707963267948966192);
    const complex<double> exp_phase_shift = exp(phase_shift);
    ...
    if (one_over_z != 0.0)
    {
        ...
        if (isfinite(log_cut_constant_AS) && (real(z) < 0.0) && (SIGN(imag(z)) == -omega))
        {
            const complex<double> factor =
                exp(-two_I_omega * (z - eta * (0.69314718055994530942 + log(z))) - phase_shift + log_cut_constant_AS);
...
// ------------------------------------------------------------------
// To calculate H[omega](z) and H'[omega](z), one integrates numerically H[omega]''(z) = [l(l+1)/z^2 + 2.eta/z -
// 1].H[omega](z) starting from debut,H_debut = H[omega](debut) and dH_debut = H'[omega](debut). There is no branch cut
// problem as Re[z] >= 0. The starting point comes for the regular function from the stored values debut, F_debut and
// dF_debut. If debut = 0, one puts debut = debut_omega = z/|2z| and calculates F(debut) and F'(debut) with power
// series. Then, the starting point {debut_omega,H[omega](debut),H'[omega](debut)} is calculated from
// {debut,F(debut),F'(debut)} and the continued fraction h[omega](debut). The first order expansions method is used if
// one is very close to the real axes of l,eta and z (Re[z] > 0). The step of the integration is (z - debut)/N_num, with
// N_num = [|z - debut|/min (0.1,10.turning_point)] + 1. The value of min (0.1,10/turning_point) gives a Smaller step
// when turning_point increases, as calculations become there more difficult as |H[omega]| typically varies faster in
// this case. The intermediates points are called z_aft. They go from debut to z, and (debut_omega,H_debut,dH_debut) is
// put to {z_aft,H[omega](z_aft),H'[omega](z_aft)} at each step. If |H[omega]| increases along the path, the integration
// is stable. If is_H_dir_int_naive is true, one has to integrate forward, as this integration is used to calculate the
// continued fraction. If not, and if |H[omega]| decreases, one integrates backwards from z_aft to debut_omega with the
// knowledge of h[omega](z_aft) = H'[omega](z_aft)/H[omega](z_aft). H'[omega](z_aft)/H[omega](z_aft) is given by the
// continued fraction formula. One then obtains by direct integration C.H[omega](debut) and C.H'[omega](debut). Knowing
// H_debut, one deduces H[omega](z) = 1/C and H'[omega](z) = f(z_aft).H[omega](z). Increase or decrease is known using
// the Taylor expansion of H[omega] near debut_omega in z up to second order.
//
...
// z_aft : next point in which H[omega] is calculated by direct integration . It is z - i*step_num, with i from N_num-1
// to 0. one_over_debut,log_H_debut_der,d2H_debut_over_H_debut: 1/debut, H'[omega](debut)/H[omega](debut),
...
//                                          after integration from the starting point {z_aft, 1.0,
//                                          H'[omega](z_aft)/H[omega](z_aft)}. Then, H[omega](z_aft) =
//                                          H_debut/H_debut_not_normed and H'[omega](z_aft) = h[omega].H[omega](z_aft) .

void Coulomb_wave_functions::H_dH_direct_integration(const int omega, const complex<double> &z, complex<double> &H,
                                                     complex<double> &dH)
{
    ...
    if (((y != 0.0) || (eta_i != 0.0) || (l_i != 0.0)) && (abs(y) < sqrt_precision * min(1.0, x)) &&
        (abs(eta_i) < sqrt_precision) && (abs(l_i) < sqrt_precision) &&
    ...
    const complex<double> step_num = (z - debut_omega) / static_cast<double>(N_num), ll_plus_one = l * (l + 1.0),
                          two_eta = 2.0 * eta;

    for (unsigned int i = N_num - 1; i <= N_num; i--)
    {
        const complex<double> z_aft = z - i * step_num, one_over_debut = 1.0 / debut,
        ...
        if (!isfinite(H) || !isfinite(dH))
            cout << "Numerical failure encountered in H_dH_direct_integration." << endl, exit(1);
...
// one has to separate the calculation of the real and imaginary parts of H[omega] and H'[omega], as they can differ by
...
void Coulomb_wave_functions::H_dH_from_first_order_expansions(const int omega, const complex<double> &z,
                                                              complex<double> &H, complex<double> &dH)
{
...
// If 1+l+i.omega.eta is negative integer, H[omega] is proportional to F so the values F and F' are arbitrarily chosen
// for H[omega] and H'[omega].
//
...
// If |f - h[sign(Im[z])]|oo >= 1, F and H[sign] are numerically linearly independent so the continued fraction is
// meaningful. Then, h[-sign(Im[z])](z) is not needed and is put to f, the worst value it can have. If not,
// h[-sign(Im[z])](z) is needed and calculated. Then, h[omega] and h[-omega] take their values from h[sign(Im[z])]| and
// h[-sign(Im[z])]|.
//
// If |f - h[omega]|oo > |f - h[-omega]|oo, one uses the continued fraction h[omega].
// If Re[z] > 0 or sign(Im[z]) = omega (good quadrants), H[omega] = 1/F/(f - h[omega]) and H'[omega] =
// h[omega].H[omega]. If not, one has to take the branch cut into account, as one is in the bad quadrant of H[omega] :
...
// If log [cut_constant] is not finite, it means that cut_constant is exactly zero if it is defined, so that there is no
// branch cut to consider.
//
// If |f - h[omega]|oo < |f - h[-omega]|oo, one uses the continued fraction h[-omega],
...
// cut_constant_CFa,log_cut_constant_CFa : cut constant for H[omega], so in its bad quadrant H[omega](z) =  1/F/(f -
// h[omega]) - cut_constant.F and its log.
//                                         cut_constant is cut_constant_CFa_plus if omega = 1, cut_constant_CFa_minus if
//                                         omega = -1. If the log is not finite, cut_constant has to be strictly zero if
//                                         it is defined so there is no branch cut to consider.
// H_minus_omega,dH_minus_omega : H-(z) and H-'(z) if omega=1, H+(z) and H+'(z) if omega=-1 in good quadrants.
//                                In bad quadrants, they are not equal to the previous functions, but combined with
//                                branch cut formulas, one calculates them.
// cut_constant_CFb,constant,norm_functions : cut_constant is cut_constant_CFb_plus if omega = 1, cut_constant_CFb_minus
// if omega = -1.
//                                            constant is 2.i.omega.norm_functions in good quadrants, cut_constant in
//                                            bad quadrants. norm_functions is 1 for normalized functions, C(l,eta)^2
//                                            for unnormalized wave functions. One has : H[-omega](z) =  1/F/(f -
//                                            h[-omega]) + constant.F in all quadrants.
// log_cut_constant_CFb,log_constant,log_norm : logs of previous values.
// log_two_I_omega : log[2.i.omega] = log[2] + i.omega.Pi/2 .

void Coulomb_wave_functions::H_dH_with_F_dF_and_CF(const int omega, const complex<double> &z, complex<double> &H,
                                                   complex<double> &dH)
{
    complex<double> F, dF;
    F_dF(z, F, dF);
    ...
    {
        const complex<double> h_sign = continued_fraction_h(z, SIGN(y)),
                              h_minus_sign = (abs(f - h_sign) < 1.0) ? (continued_fraction_h(z, -SIGN(y))) : (f);
        const complex<double> h_omega = (omega == SIGN(y)) ? (h_sign) : (h_minus_sign),
        ...
        if (inf_norm(f - h_omega) > inf_norm(f - h_minus_omega))
        {
            ...
            const complex<double> log_cut_constant_CFa =
                (omega == 1) ? (log_cut_constant_CFa_plus) : (log_cut_constant_CFa_minus);

            if ((isfinite(log_cut_constant_CFa)) && (x < 0.0) && (SIGN(y) == -omega))
            {
                const complex<double> cut_constant_CFa =
                    (omega == 1) ? (cut_constant_CFa_plus) : (cut_constant_CFa_minus);
        ...
        {
            const complex<double> H_minus_omega = 1.0 / (F * (f - h_minus_omega)),
            ...
            const complex<double> constant =
                ((x < 0.0) && (SIGN(y) == omega)) ? (cut_constant_CFb) : (two_I_omega * norm_functions);

            if ((constant == 0.0) || (!isfinite(constant)))
            {
                const complex<double> log_norm = (!is_it_normalized) ? (2.0 * log_Cl_eta) : (0.0),
                                      log_two_I_omega(0.69314718055994530942, omega * 1.5707963267948966192);
                const complex<double> log_cut_constant_CFb =
                    (omega == 1) ? (log_cut_constant_CFb_plus) : (log_cut_constant_CFb_minus);
                const complex<double> log_constant =
                    ((x < 0.0) && (SIGN(y) == omega)) ? (log_cut_constant_CFb) : (log_two_I_omega + log_norm);

                H = exp(log_constant + log(F)) + H_minus_omega, dH = exp(log_constant + log(dF)) + dH_minus_omega;
            }
            else
                H = constant * F + H_minus_omega, dH = constant * dF + dH_minus_omega;
        }
    }
}
...
// H[omega] = (exp[i.omega.chi].F - Fp)/sin (chi), H'[omega] = (exp[i.omega.chi].F' - Fp')/sin (chi) if wave functions
// are normalized, H[omega] = (exp[i.omega.chi].F.C(l,eta)^2/sin (chi) + Fp/(2l+1), H'[omega] =
// (exp[i.omega.chi].F'.C(l,eta)^2/sin (chi) + Fp'/(2l+1) if not. chi is sigma(l,eta) - sigma(-l-1,eta) - (l+0.5).Pi,
// and Fp is F(-l-1,eta). Fp is calculated using a class Coulomb_wave_functions with parameters -l-1 and eta. To avoid
// numerical imprecisions, sin (chi) is calculated with the stable formula -(2l+1).C(l,eta).C(-l-1,eta). The validity of
// this expansion is checked with the wronskian of F and Fp, which must be correct up to precision : F'.Fp - Fp'.F = sin
// (chi) if wave functions are normalized, Fp'.F - F'.Fp = 2l + 1 if not. If the wronskians are numerically correct, one
// does the calculation and is_it_successful is put to true. If not, one puts is_it_successful to false and quits the
// routine. If C(l,eta)^2 underflows or overflows, one uses logs of F,F' and C(l,eta)^2 for the calculation.
//
// Variables
// ---------
// omega : 1 if one calculates H+(z) and H+'(z), -1 if one calculates H-(z) and H-'(z).
...
// H,dH : H+(z) and H+'(z) if omega=1, H-(z) and H-'(z) if omega=-1.
// is_it_successful : false if the wronskian between F(l,eta,z) and F(-l-1,eta,z) is not equal to zero up to precision,
// true if not. Fp,dFp : F(-l-1,eta,z), F'(-l-1,eta,z). exp_I_omega_chi : exp[i.omega.chi] one_over_2lp1 : 1/(2l + 1)
...
void Coulomb_wave_functions::H_dH_with_expansion(const int omega, const complex<double> &z, complex<double> &H,
                                                 complex<double> &dH, bool &is_it_successful)
{
    if (cwf_lp_ptr == 0)
        cwf_lp_ptr = new class Coulomb_wave_functions(is_it_normalized, -l - 1, eta);
    ...
    else
    {
        ...
        {
            const complex<double> Cl_eta_2 = Cl_eta * Cl_eta,
                                  exp_I_omega_chi_over_sin_chi = exp_I_omega_chi * one_over_sin_chi;
            const complex<double> F_Cl_eta_2 =
                ((Cl_eta_2 == 0.0) || (!isfinite(Cl_eta_2))) ? (exp(2.0 * log_Cl_eta + log(F))) : (Cl_eta_2 * F);
            const complex<double> dF_Cl_eta_2 =
                ((Cl_eta_2 == 0.0) || (!isfinite(Cl_eta_2))) ? (exp(2.0 * log_Cl_eta + log(dF))) : (Cl_eta_2 * dF);
...
// If there is overflow or underflow for C(l,eta) in this last case, one uses logs of F,F' and C(l,eta) for the
// calculation.
//
// Variables:
// ----------
...
// The test of convergence is |(n+l-1).a[n-2]|oo + |(n+l).a[n-1]|oo, as one of the two can be zero even before
// convergence.
...
void Coulomb_wave_functions::F_dF_power_series(const complex<double> &z, complex<double> &F, complex<double> &dF)
{
    if (z == 0.0)
    {
        if (l == 0)
            ...
            cout << "F(z=0) and/or F'(z=0) are undefined." << endl, abort();
    }
    else
    {
...
// b[0] = l + 1 + i.omega.z, a[n] = -2.i.omega.[1 + l + i.omega.eta] + (n-1).[-2.i.omega.z], b[n] = 2l + 2 + 2.i.omega.z
// + n-1. omega is 1 or -1, and theoretically the result is the same. If they are not equal numerically, omega =
// sign[-Im [z]] gives usually the best result. If 1+l+i.omega.eta is a negative integer, f[omega] is finite and must be
// used.
//
// Variables:
// ----------
...
complex<double> Coulomb_wave_functions::continued_fraction_f(const complex<double> &z, const int omega)
{
    ...
    const complex<double> minus_two_I_omega_z = -2.0 * I_omega * z, minus_two_I_omega_a_z = minus_two_I_omega_z * a,
...
//
//
// S+ and S- and derivatives are calculated in asymptotic_series. If is_it_successful is true, the series are
// meaningful. If not, one leaves the routine. Norm is C(l,eta) if one uses normalized wave functions, 1.0 if not. If
// there is overflow or underflow for C(l,eta) in this last case, one uses logs of F,F' and C(l,eta) for the
// calculation.
//
// Variables :
// -----------
...
// is_it_successful : true if the calculation converged, i.e. the series are good up to precision and the wronskian of
// H+,H- up to precision, false if not. sum,dsum : asymptotic series. sum[0] = S+(z), sum[1] = S-(z), dsum[0] = S+'(z),
// dsum[1] = S-'(z). I,one_over_two_I : i,1/[2i]. I_one_minus_eta_over_z : i*(1 - eta/z). exp_phase_shift_plus : exp
// (i*(z - eta.log(2z) - l.Pi/2 + sigma(l,eta)))). exp_phase_shift_minus : exp (-i*(z - eta.log(2z) - l.Pi/2 +
// sigma(l,eta)))). H_plus,dH_plus,H_minus,dH_minus : exp (+/-i*(z - eta.log(2z) - l.Pi/2 + sigma(l,eta))).S(+/-)(z) and
// derivatives.

void Coulomb_wave_functions::asymptotic_expansion_F_dF(const complex<double> &z, complex<double> &F,
                                                       complex<double> &dF, bool &is_it_successful)
{
    complex<double> sum[2], dsum[2];
    ...
    const complex<double> exp_phase_shift_plus =
        exp(I * (z - eta * (0.69314718055994530942 + log(z)) - l * 1.5707963267948966192 + sigma_l));
    const complex<double> exp_phase_shift_minus = 1.0 / exp_phase_shift_plus;

    const complex<double> H_plus = sum[0] * exp_phase_shift_plus,
                          dH_plus = (dsum[0] + sum[0] * I_one_minus_eta_over_z) * exp_phase_shift_plus;
    const complex<double> H_minus = sum[1] * exp_phase_shift_minus,
                          dH_minus = (dsum[1] - sum[1] * I_one_minus_eta_over_z) * exp_phase_shift_minus;
...
// The intermediates points are called z_aft. They go from debut to z, and (debut,F_debut,dF_debut) is put to
// {z_aft,F(z_aft),F'(z_aft)} at each step. If |F| increases along the path, the integration is stable. If it decreases,
// and if z_aft decreases in modulus or one does not integrate with constant argument (i.e. theta constant in z =
// |z|.exp[i.theta]) for Re[l] > -1, one reintegrates F(z) from debut = z/|2z|, as integration is usually stable at
// constant argument for Re[l] > -1. Increase or decrease is known using the Taylor expansion of F near debut in z up to
// second order. If one integrates with constant argument and |F| decreases, one integrates backwards from z_aft to
// debut with the knowledge of f(z_aft) = F'(z_aft)/F(z_aft). F'(z_aft)/F(z_aft) is given by the continued fraction
// formula. One then obtains by direct integration C.F(debut) and C.F'(debut). Knowing F_debut, one deduces F(z) = 1/C
// and F'(z) = f(z_aft).F(z). If 1+l+i.omega.eta is a negative integer, f[omega] is finite and is used. Otherwise,
// f(z_aft) is calculated with omega = 1 and -1. If they are equal up to precision, f(omega) is correct and used. omega
// is chosen so Re[-2.i.omega.z] < 0, for which the anomalous convergence phenomenon of Gautschi of f is the Smallest
// (W. Gautschi, Math. Comp. Vol. 31 p.994). If not, but |norm.F| < 0.1, one still has to use f[omega], as it is
// probably correct as F is the minimal solution, and also one has no other way to calculate F. If |norm.F| > 0.1 in
// this case, one stops the procedure and F will be calculated from H+ and H-, given by direct integration and continued
// fraction formulae. In this case, is_it_successful is put to false, and otherwise the integration worked and it is put
// to true. Norm is 1.0 if one uses normalized wave functions, C(l,eta) if not.
//
...
// i.e. |F| > 0.1 decreasing on the integration path and f(omega) is not equal to f(-omega) up to precision, true if
// not. ODE : reference to pointer ODE_ptr, which performs direct integration from one point to another. step_abs :
// length of the integration step from z to debut. It is min (0.1,10/turning_point) N_num : number of integrations to do
// from debut to z. It is |z - debut|/step_abs + 1. step_num : complex step from debut to z. It is (z - debut)/N_num .
// ll_plus_one,two_eta : l(l+1), 2.eta. They are used in the Taylor expansion test.
// z_aft : next point in which F is calculated by direct integration . It is z - i*step_num, with i from N_num-1 to 0.
// one_over_debut,log_F_debut_der,d2F_debut_over_F_debut: 1/debut, F'(debut)/F(debut), F''(debut)/F(debut). They are
// used in the Taylor expansion test. ratio : debut/z. It is used to know if one integrates with constant argument or if
// |z_aft| increases.
...
// fp,fm : continued fractions F'/F with omega = 1 or -1 used in the case of 1+l +/- i.eta negative integer, as
// continued fractions are finite in this case. norm_functions : C(l,eta)^2 for unnormalized functions, 1 for normalized
// functions. F_debut_not_normed,dF_debut_not_normed : unnormed regular wave function and derivative in debut,
...
void Coulomb_wave_functions::F_dF_direct_integration(const complex<double> &z, complex<double> &F, complex<double> &dF,
                                                     bool &is_it_successful)
{
    if (z == debut)
    {
    ...
    if (debut == 0.0)
        debut = 0.5 * z / abs(z), F_dF_power_series(debut, F_debut, dF_debut);

    const class ODE_integration &ODE = *ODE_ptr;
    const double step_abs = min(0.1, 10.0 / turning_point);
    const unsigned int N_num = static_cast<unsigned int>(rint(abs(z - debut) / step_abs) + 1);
    const complex<double> step_num = (z - debut) / static_cast<double>(N_num), ll_plus_one = l * (l + 1.0),
                          two_eta = 2.0 * eta;
    const complex<double> norm_functions = (!is_it_normalized) ? (Cl_eta * Cl_eta) : (1.0);
    ...
    for (unsigned int i = N_num - 1; i <= N_num; i--)
    {
        ...
        if (abs(1.0 + step_num * (log_F_debut_der + 0.5 * step_num * d2F_debut_over_F_debut)) < 1.0)
        {
            ...
            {
                debut = 0.0, F_dF_direct_integration(z, F, dF, is_it_successful);
                return;
            }
            ...
            {
                const complex<double> f_omega = continued_fraction_f(z_aft, SIGN(-imag(z_aft))),
                                      f_minus_omega = continued_fraction_f(z_aft, -SIGN(-imag(z_aft)));

                //// Comment the following line if you want to accept the value of f(omega) = F'/F inconditionally
                if ((abs(F * norm_functions) > 0.1) && (abs(f_minus_omega / f_omega - 1.0) > precision))
                {
                    is_it_successful = false;
                    return;
                }
        ...
        if (!isfinite(F) || !isfinite(dF))
            cout << "Numerical failure encountered in F_dF_direct_integration." << endl, exit(1);
    }
    is_it_successful = true;
}
...
// : F(l,eta,z) = -F(l,-eta,-z).exp[-Pi.(eta-i.l)], F'(l,eta,z) = F'(l,-eta,-z).exp[-Pi.(eta-i.l)] if arg (z) > 0 and
// is_it_normalized is true, F(l,eta,z) = -F(l,-eta,-z).exp[-Pi.(eta+i.l)], F'(l,eta,z) =
// F'(l,-eta,-z).exp[-Pi.(eta+i.l)] if arg (z) <= 0 and is_it_normalized is true, F(l,eta,z) =
// -F(l,-eta,-z).exp[i.Pi.l)], F'(l,eta,z) = F'(l,-eta,-z).exp[i.Pi.l)] if arg (z) > 0 and is_it_normalized is false,
// F(l,eta,z) = -F(l,-eta,-z).exp[-i.Pi.l)], F'(l,eta,z) = F'(l,-eta,-z).exp[-i.Pi.l)] if arg (z) <= 0 and
...
void Coulomb_wave_functions::F_dF_with_symmetry_relations(const complex<double> &z, complex<double> &F,
                                                          complex<double> &dF)
{
    ...
    if ((debut != 0.0) && (cwf_minus_eta_ptr->debut != -debut))
    {
        ...
        const complex<double> sym_constant_debut =
            (arg_debut <= 0.0) ? (1.0 / sym_constant_arg_neg) : (1.0 / sym_constant_arg_pos);

        cwf_minus_eta_ptr->debut = -debut;

        if ((sym_constant_debut == 0.0) || (!isfinite(sym_constant_debut)))
        {
            const complex<double> log_sym_constant_debut =
                (arg(debut) <= 0.0) ? (-log_sym_constant_arg_neg) : (-log_sym_constant_arg_pos);
    ...
    {
        const complex<double> log_sym_constant =
            (arg_z <= 0.0) ? (log_sym_constant_arg_neg) : (log_sym_constant_arg_pos);
...
// S(+/-)(z) = 1.0+\sum_(n=1)^N a[n] with a[n+1]=a[n].[n.[n+1+/-2i.eta]+i.eta.(i.eta+/-1)-l(l+1)]/[+/-2i.(n+1)]/z, n >=
// 0 and a[0] = 1. This expansion diverges : it is only useful with the Smallest term summation method. The test of
// convergence is max(|a[n]|oo,|n.a[n]/z|oo), so the largest norm of the term of series of function and derivative.
...
// is_it_successful : true if the series converged and |wronskian - 2.i.omega|oo is Smaller than precision, false if
// not. test : test of convergence of the asymptotic series. It is max(|a[n]|oo,|n.a[n]/z|oo). sign : if i=0, it is
// omega, if i=1, it is -omega. Ieta,two_I_eta_sign : i.eta, 2.i.eta if sign = 1, -2.i.eta if sign = -1.
                                               ...
                                               complex<double> sum[], complex<double> dsum[], bool &is_it_successful)
{
    ...
    for (unsigned int i = 0; i <= 1; i++)
    {
        ...
        const complex<double> Ieta(-imag(eta), real(eta)),
            ...
            Ieta_Ieta_plus_sign_minus_ll_plus_one = Ieta * (Ieta + sign) - l * (l + 1.0);
        ...
        do
        {
            ...
            an_sign *= one_over_z * (n * (n_plus_one + two_I_eta_sign) + Ieta_Ieta_plus_sign_minus_ll_plus_one) *
    ...
    is_it_successful = (inf_norm(sum[1] * dsum[0] - sum[0] * dsum[1] +
                                 two_I_omega * (1.0 - eta * one_over_z) * sum[0] * sum[1] - two_I_omega) < precision);
}
...
// cwf_plus,cwf_minus : references on class Coulomb_wave_functions with parameters l+eps,eta and l-eps,eta or l,eta+eps
// and l,eta-eps.
//                      If one calculates the partial derivatives according to eta, they are *cwf_real_eta_plus_ptr and
//                      *cwf_real_eta_minus_ptr. If one calculates the partial derivatives according to l, they are
//                      *cwf_real_l_plus_ptr and *cwf_real_l_minus_ptr.
// x : Re[z], z the argument of the wave function.
// d_chi_Bx,d_chi_dBx : partial derivatives of F and F' in l_r,eta_r,x according to chi = l or eta if is_it_regular is
// true, or G and G' if not. l_r,eta_r,chi_r : Re[l],Re[eta], Re[eta] is is_it_eta is true, Re[l] if not.
...
//                                   H+(x,chi+eps),H+'(x,chi+eps),H+(x,chi-eps),H+'(x,chi-eps) if is_it_regular is
//                                   false.
...
void Coulomb_wave_functions::partial_derivatives(const bool is_it_regular, const bool is_it_eta, const double x,
                                                 double &d_chi_Bx, double &d_chi_dBx)
{
    const double l_r = real(l), eta_r = real(eta), chi_r = (is_it_eta) ? (eta_r) : (l_r);
    const double chi_r_plus =
        (chi_r != 0.0) ? (chi_r * (1.0 + prec_first_order_expansion)) : (prec_first_order_expansion);
    ...
    if (is_it_eta)
    {
        if (cwf_real_eta_plus_ptr == 0)
            cwf_real_eta_plus_ptr = new class Coulomb_wave_functions(is_it_normalized, l_r, chi_r_plus);
        if (cwf_real_eta_minus_ptr == 0)
            cwf_real_eta_minus_ptr = new class Coulomb_wave_functions(is_it_normalized, l_r, chi_r_minus);
    }
    else
    {
        if (cwf_real_l_plus_ptr == 0)
            cwf_real_l_plus_ptr = new class Coulomb_wave_functions(is_it_normalized, chi_r_plus, eta_r);
        if (cwf_real_l_minus_ptr == 0)
            cwf_real_l_minus_ptr = new class Coulomb_wave_functions(is_it_normalized, chi_r_minus, eta_r);
    }
...
// ------------------------------------------------------------------------------
// When imaginary parts of l,eta,z are much Smaller than their real parts but not all zero, with Re[z] > 0,
// one has to separate the calculation of the real and imaginary parts of (F,G)(z) and (F',G')(z), as they can differ by
// tens of orders of magnitude. Re[z] < 0 is not considered, as G(z) is complex with Im[z] = Im[l] = Im[eta] = 0. So,
// with z = x+i.y, eta = eta_r + i.eta_i and l = l_r+i.l_i, one calculates (F,G)[l_r,eta_r](x) and (F,G)'[l_r,eta_r](x).
//
// One considers here the parameters l_r and eta_r with the argument x, and the parameters l and eta with the argument
// z.
//
...
//
// After that, one expands (F,G)(z) and (F',G')(z) in first order in y, eta_i and l_i and one has, up to y^2,eta_i^2 and
// l_i^2:
//
...
//                                     They are initialized at zero, and not calculated if they are multiplied by zero
//                                     after.
// Coulomb_eq_x,d2Bx : l_r(l_r+1)/[x^2] + 2.eta_r/x - 1, B''(x) = [l_r(l_r+1)/[x^2] + 2.eta_r/x - 1].B(x),

void Coulomb_wave_functions::first_order_expansions(const bool is_it_regular, const complex<double> &z,
                                                    complex<double> &B, complex<double> &dB)
{
    const double x = real(z), y = imag(z), l_r = real(l), l_i = imag(l), eta_r = real(eta), eta_i = imag(eta);

    if (cwf_real_ptr == 0)
        cwf_real_ptr = new class Coulomb_wave_functions(is_it_normalized, l_r, eta_r);
...
void Coulomb_wave_functions::F_dF(const complex<double> &z, complex<double> &F, complex<double> &dF)
{
        ...
        else
        {
            ...
            if (!is_it_successful)
            {
                ...
                if (!neg_int_omega_one && !neg_int_omega_minus_one && !is_it_successful)
                {
                    ...
                    const complex<double> two_I_omega(0.0, 2.0 * omega),
                        two_I_term = (is_it_normalized) ? (two_I_omega) : (two_I_omega * Cl_eta * Cl_eta);
                    const complex<double> h_omega = continued_fraction_h(z, omega),
                    ...
                    const complex<double> H_minus_omega = two_I_term / ((h_omega - h_minus_omega) * H_omega),
                                          dH_minus_omega = h_minus_omega * H_minus_omega;
    ...
    if (!isfinite(F) || !isfinite(dF))
        cout << "Numerical failure encountered in F_dF." << endl, exit(1);
...
// If l,eta and z are real, one rewrites H[omega] as H[omega] = Re[H[omega]] + i.omega.norm.Re[F] to avoid numerical
// inaccuracies for Im[H[omega]]. norm is 1 if is_it_normalized is true, C(l,eta)^2 if not.
//
//
// Variables :
// -----------
...
//                    true if the expansion formula with F(l,eta,z) and F(-l-1,eta,z) worked, false it not (in
//                    H_dH_with_expansion).
// H_scaled,dH_scaled : H[omega](z).exp[-i.omega.[z - eta.log[2z]]) and H'[omega](z).exp[-i.omega.z.[z - eta.log[2z]])
// given by the asymptotic expansion formula. I_omega_z,log_unscale,unscale : i.omega,i.omega.[z -
// eta.log[2z]],exp[i.omega.[z - eta.log[2z]]] x,y,l_r,l_i,eta_r,eta_i : Re[z], Im[z], Re[l], Im[l], Re[eta], Im[eta].
...
//                        One indeed has H[omega] = G + I.omega.norm.F, with in this case G and omega.norm.F as its real
//                        and imaginary parts.
...
void Coulomb_wave_functions::H_dH(const int omega, const complex<double> &z, complex<double> &H, complex<double> &dH)
{
    ...
    if (!neg_int_omega_one && !neg_int_omega_minus_one)
        asymptotic_expansion_H_dH_scaled(omega, 1.0 / z, H_scaled, dH_scaled, is_it_successful);

    if (is_it_successful)
    {
        const complex<double> I_omega(0, omega), log_unscale = I_omega * (z - eta * (0.69314718055994530942 + log(z))),
    ...
    else
    {
        ...
        {
            //// Replace the following line by : if (!neg_int_omega_one && !neg_int_omega_minus_one) H_dH_with_expansion
            ///(omega,z,H,dH,is_it_successful); / if you want H_dH_with_expansion to be used if |l_i| < 1 or |z| > 1.
            if (!neg_int_omega_one && !neg_int_omega_minus_one && (abs(l_i) >= 1.0) && (abs(z) <= 1.0))
            ...
            if ((y == 0.0) && (eta_i == 0.0) && (l_i == 0.0))
            {
                ...
                const double omega_norm_functions =
                    (!is_it_normalized) ? (omega * real(Cl_eta) * real(Cl_eta)) : (omega);
    ...
    if (!isfinite(H) || !isfinite(dH))
        cout << "Numerical failure encountered in H_dH." << endl, exit(1);
}
...
// If not, or if it failed, one uses the continued fraction formula.
// If l,eta and z are real, one rewrites H[omega] as H[omega] = Re[H[omega]] + I.omega.norm.Re[F], to avoid numerical
// inaccuracies for Im[H[omega]]. norm is 1 if is_it_normalized is true, C(l,eta)^2 if not. One uses logs if the scaling
// factor underflows or overflows.
//
// Variables :
// -----------
...
void Coulomb_wave_functions::H_dH_scaled(const int omega, const complex<double> &z, complex<double> &H_scaled,
                                         complex<double> &dH_scaled)
{
    bool is_it_successful = false;
    if (!neg_int_omega_one && !neg_int_omega_minus_one)
        asymptotic_expansion_H_dH_scaled(omega, 1.0 / z, H_scaled, dH_scaled, is_it_successful);
    ...
    if (!is_it_successful)
    {
        ...
        const complex<double> I_omega(0, omega), log_scale = -I_omega * (z - eta * (0.69314718055994530942 + log(z))),
                                                 scale = exp(log_scale);
    ...
    if (!isfinite(H_scaled) || !isfinite(dH_scaled))
        cout << "Numerical failure encountered in H_dH_scaled." << endl, exit(1);
}
